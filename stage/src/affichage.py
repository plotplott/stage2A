...
def affichage_poids_Dice(chemin):
    ...
    for i in file_list:
        ...
        liste_cnn.append([taille_tumeur(matCNN), Frame_Performances.indice_dice(matExpert, matCNN)])
    liste_cnn = sorted(liste_cnn)
    print(liste_cnn)
    print("J'ai fini CNN")
    ...
    for i in file_list:
        ...
        liste_ovassion.append([taille_tumeur(matOvassion), Frame_Performances.indice_dice(matExpert, matOvassion)])
    liste_ovassion = sorted(liste_ovassion)
    print(liste_ovassion)
    print("J'ai fini Ovassion")
    '''
    # on créé la liste pour l'affichage du chemin
    liste_chemin = []
    ...
    for i in file_list:
        ...
        liste_chemin.append([taille_tumeur(matChemin), Frame_Performances.indice_dice(matExpert, matChemin)])
    liste_chemin = sorted(liste_chemin)
    print(liste_chemin)
    print("J'ai fini chemin")
    '''
    plt.plot(liste_cnn, label='CNN pur')
    plt.plot(liste_ovassion, label = 'Ovassion pur')
    plt.scatter(liste_chemin, label='Méthode testée')
    plt.xlim(19, 131)
    plt.legend()
    plt.xlabel('Taille de la tumeur')
    plt.ylabel('Indice de Dice')
    #plt.title("Représentation de l'indice de Dice avec la méthode de convolution pour les poids 0.5-0.3-0.2 \navec la tumeur et le rein")
    plt.show()
    '''
...
affichage_poids_Dice("../Data/basic/red/cnn_cnn/tumeur/")